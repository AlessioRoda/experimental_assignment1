...
The purpose of this repository is to generate a simulation for the Cluedo game, in which the robot moves from a room to another in the Gazebo simulation, searches for ArUco markers that provide the hints to determine the solution to the Cluedo game and tries to provide a solution basing on the collected hints, until the correct solution is found. The scenario in which the robot moves is composed by six different rooms in which there are five ArUco markers; the robot is able to move in the environment mapping it with the laser_scan that is mounted on its basis in order to generate a global map of the area. Also, the robot has a manipulator in which is mounted a camera on the end effector that is needed to detect the markers: the robot continuously moves the arm in order to visualize all the markers in the room, since they are in a not defined position and orientations.
...
* exp_assignment3: package already provided for the simulation (here the link https://github.com/CarmineD8/exp_assignment3/)
...
Concerning the aruco_ros and moveit_config packages only small changes have been performed: for the first one a publisher of the marker ID has been added inside aruco_ros/aruco_ros/marker_publish.cpp, instead in the second one some small changes were needed to change the default Gazebo simulation environment, generated by the setup assistant, to the new simulation environment provided for the Cluedo game.
...
* scripts folder containing the ontology_interface.py script to manage the queries for the ARMOR ontology and the state_machine.py script that implements the SMACH state machine that represents the behavior of the robot during the simulation. It also contains a sub folder with the MyArmor and Place classes.
...
* Receives all the maker IDs that are detected by the aruco_ros node via /marker_id service, it saves and send them to the final_oracle node, so to receive the Cluedo hint as a response 
...
* During the TRY_SOLUTION state it asks the Cluedo solution ID to the final_oracle node by sending the /oracle_solution request and compares the service response that receives with all the complete and consistent IDs that have received before. If one of them is the solution of the cluedo game, it asks the person, the weapon and the place of the solution by calling the /ontology_interface/ask_solution service, then the simulation ends; otherwise, in case the IDs don't correspond to the solution, the simulation continues again from MOVE state
...
* When the /ontology_interface/add_hint request is received, it stores the elements of the hints in three different lists 
...
* When /ontology_interface/ask_solution service request is received, it uses armor commands to query the person, the weapon and the place of the cluedo game given the solution ID provided in the request

All the armor operations are performed by calling the MyArmor class, a custom class that provides a more agile way to send armor messages (for more information about, please have a look at the sphinx documentation)
...
### Temporal diagram
![temporal_diagram drawio](https://user-images.githubusercontent.com/48511957/192269712-88cfb145-b362-458e-868c-823b7d40d24c.png)

### SMACH FSM

![smach](https://user-images.githubusercontent.com/48511957/192152132-9fc66550-da8b-42eb-8cde-c38433b1d203.PNG)
 The simulation starts in the EXPLORE state, here a MoveBase goal is sent with the coordinates of one of the rooms in the environment that haven't been already visited or, in case the flag variable to determine if the robot must go in the oracle room or not is equal to True, it moves to the oracle room. The state waits until the action is completed, then it will wait five seconds to let the arm explore the area with the camera searching the markers. After that, the FSM goes in the CHECK_CONSISTENCY state, in which it sends a service message on topic /oracle_hint for each marker ID that has been collected until that moment (and that wasn't already tried: each ID is considered just once) and adds the corresponding hints in the ontology. Then it asks for all the complete and consistent hypothesis in the ontology and removes the inconsistent ones. In case there are any complete and consistent hypothesis, it updates a flag to go in the oracle room, then returns in the EXPLORE state.
Finally, when robot reaches the oracle room, it enters in the TRY_SOLUTION state and checks if one of the possible solutions that have found is the real oracle solution to the cluedo game; in case it is, it asks for the person, the weapon and the place of the solution in order to print them, then the game ends and the FSM returns 'correct', otherwise it returns in the EXPLORE state.
