...
The first one contains the Sphinx documentation where you can find a detailed explanation about all the code, you can also read it by accessing to this link: https://alessioroda.github.io/experimental_assignment1/ 
...
The srv folder contains the service custom messages: AskHint that returns three arrays for the people, the weapons and the places collected in the hint and the ID of the hint, Move that sends the actual position and the final position to reach expressed in x and y coordinate, then returns a boolean to confirm if the 
...
The MyArmor class comunicates with the Update Ontology node by sending an ARMOR message and receiving the ARMOR response that passes to the state_machine node.
...
```
You will see all the logs of the state_machine node, but the ones of the other nodes of the simulation will not appear in order to make the simualtion easier to follow on the terminal.
...
Here as befor all the nodes are launched and just state_machine plots the logs, but in this case you can also see the simulation switching from a state to another in the FSM thanks to the smach viewer.
...
In according to what has been reported for the description of the architecture, it's important to notice that the purpose of the cluedo simulation is to stress the usage of ARMOR: in this sense the technical choices for the genreation of the hints and the mechanisms to deriver the solution are deliberately made for asking continusly the oracle for hints untill one of them is not the correct solution. For this reason the oracle can generate hints that often are incomplete, envenmore when the oracle gets the solution it does not provide any information if one of the elements between PERSON, PLACE and WEAPON are correct, so there isn't any deduction in the state machine_node to faster derive the right solution.
For this reason since the big amount of movements that the robot has to perform before getting the hint that corresponds to the solution and since for each movement corresponds a certain time in which the simulation has to wait, the simulation can require a lot of time to derive the solution, even with a small amount of elements (as in this case just three people, three places and three weapons). 
In order to limit the dealy generated by searcing for queries in an ontolgy with a very big amount of hypothesis, all the hypotesys (except from the solution) are removed from the ontology as long as it is discovered that they are imcoplete, inconsistent or that they do not represent the right solution. In this way, even if the big amount of generated hints, the time required to ask for a query reamins constant and that permits to avoid slowdowns in the case in which the simulation would generate a very big number of hints.

In the case in which a faster simualtion is preferred, one possible future implementation could be the capability to remember the solutions that are wrong in order to exclude them when the oracle generates a new hint and to faster derive the correct solution. Another possibility could be the capability to understand if some of the elmenents of the solution are correct when the oracle checks it in order to ask to the oracle not to send them anymore.
