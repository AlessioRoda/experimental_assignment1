...
In order to overcome to this porblem, a control on the joint state positions of the robotic arm could be implemented, so that, when a movearm command is executed it can check if the arm is in the correct poition, otherwise the command is executed again.
